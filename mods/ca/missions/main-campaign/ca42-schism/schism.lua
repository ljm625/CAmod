...
WorldLoaded = function()
	...
	Trigger.AfterDelay(DateTime.Seconds(4), function()
		Media.DisplayMessage("停止这场疯狂。你根本不知道自己在面对什么。你会把我们所有人都带向毁灭！", "凯恩", HSLColor.FromHex("FF0000"))
		MediaCA.PlaySound(MissionDir .. "/kane_stopmadness.aud", 2)
		Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(7)), function()
			Media.DisplayMessage("你这场愚蠢的远征到此为止了，凯恩。尊王会取你首级。", "契尔坚科总理", HSLColor.FromHex("FF0000"))
...
MaleficInit = function()
	if not MaleficArrived then
		...
		Trigger.AfterDelay(DateTime.Seconds(8), function()
			Media.DisplayMessage("不可能！这些Scrin并非……绝不能让装置被摧毁！", "Scrin尊王", HSLColor.FromHex("7700FF"))
