...
WorldLoaded = function()
	...
	ObjectiveDeploySensorArrays = GDI.AddObjective("在目标位置部署传感阵列。")
	ObjectiveCaptureNerveCenter = GDI.AddObjective("夺取Scrin神经中枢。")
	...
	if Difficulty ~= "easy" then
		Trigger.AfterDelay(DateTime.Seconds(10), function()
			...
			Notification("检测到危险的泰伯利亚生命体。建议部队保持安全距离。")
		end)
	end
	...
	Trigger.OnCapture(NerveCenter1, function(self, captor, oldOwner, newOwner)
		...
		ObjectiveProtectNerveCenter = GDI.AddObjective("保护已夺取的神经中枢。")
		ObjectiveDestroyScrinBase = GDI.AddObjective("摧毁Scrin基地。")
		...
		Trigger.AfterDelay(DateTime.Seconds(3), function()
			...
			Trigger.AfterDelay(DateTime.Seconds(5), function()
				...
				Trigger.AfterDelay(AdjustTimeForGameSpeed(DateTime.Seconds(4)), function()
					...
					Notification("不要让神经中枢被摧毁，传送门必须保持稳定。")
				end)
			end)
		end)
	end)
...
CheckSensors = function()
	...
	if NumSensorsDeployed == 4 then
		...
		Trigger.AfterDelay(DateTime.Seconds(2), function()
			...
			Trigger.AfterDelay(DateTime.Seconds(9), function()
				if not SPower2.IsDead then
					...
					Notification("神经中枢受风暴之柱严密保护。传感器检测到东南方向有为这些防御供电的Scrin反应堆。")
		...
		UserInterface.SetMissionText("已部署传感阵列：" .. NumSensorsDeployed .. "/4", HSLColor.Yellow)
	end
end
